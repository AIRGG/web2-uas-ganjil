...
<div class="bg-warning-subtle">
    ...
    <form id="main-form" action="" method="post">
        ...
        <!-- <input required type="text" name="kode_kelas"><br><br> -->
        <?= inputan('', 'kode_kelas', '', 'text', '', '', 'required') ?><br><br>
        <label>Nama Kelas</label><br>
        <!-- <input required type="text" name="nama_kelas"><br><br> -->
        <?= inputan('', 'nama_kelas', '', 'text', '', '', 'required') ?><br><br>
    ...
    <script>
        ...
        //-- custom -- \\
        let form_kode_kelas = document.querySelector(`input[name="kode_kelas"]`)
        let form_nama_kelas = document.querySelector(`input[name="nama_kelas"]`)
        //-- [END] custom -- \\
        ...
        if (isValid) {
            ...
            doAjax("POST", "setup-kelas-proses.php", data, afterCRUD)
        }
        });
        ...
        const editData = (idhide, idx) => {
            ...
            // -- custom -- \\
            form_kode_kelas.value = jsndata.kode_kelas
            form_nama_kelas.value = jsndata.nama_kelas
        }
        ...
        const getData = (btn) => {
                ...
                if (that.readyState == 4 && that.status == 200) {
                    ...
                    let tbodyhtml = data.map((v, i)=> {
                        return `<tr>
                                    ...
                                    <td>${v.nama_kelas}</td>
                                    <td>
                                        <button class="btn btn-info" onclick="editData(${v.id_kelas}, ${i})">Edit</button> | <button class="btn btn-danger" onclick="return deleteData(${v.id_kelas})">Hapus</button>
                                    </td>
                                </tr>`
                    })
                    let numberpage = ''
            ...
            doAjax("GET", "setup-kelas-proses.php?view=&page="+pagenow, data, afterGetData)
        }
